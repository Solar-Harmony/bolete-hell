...
namespace BoleteHell.Gameplay.Destructible
{
    ...
    public class SpriteFragmenter : ISpriteFragmenter
    {
        ...
        public void Fragment(Transform transform, SpriteFragmentConfig config)
        {
            ...
            Vector2 rendererSize = config.renderer.bounds.size;
            var fragmentSize = new Vector2(
                Mathf.Max(config.minFragmentSize, rendererSize.x / config.fragmentsX),
                Mathf.Max(config.minFragmentSize, rendererSize.y / config.fragmentsY)
            );
            var origin = (Vector2)transform.position - rendererSize / 2;
