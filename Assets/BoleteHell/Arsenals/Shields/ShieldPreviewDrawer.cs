...
namespace BoleteHell.Arsenals.Shields
{
    ...
    {
        // Events to communicate with gameplay module
        public event Func<float, bool> OnEnergyValidationRequested;
        public event Action<float> OnEnergySpendRequested;
        ...
        [Inject]
        public void Construct(ShieldData shieldData)
        {
        ...
        public void DrawPreview(Vector3 mouseWorld)
        {
            ...
            bool canAddPoint = points.Count == 0 || 
                             (distance > spaceBetweenPoints && HasEnoughEnergy(energyRequired));
        ...
        private bool HasEnoughEnergy(float amount)
        {
            return OnEnergyValidationRequested?.Invoke(amount) ?? true;
        }

        private void SpendEnergy(float amount)
        {
            OnEnergySpendRequested?.Invoke(amount);
        }
