...
namespace BoleteHell.Utils.Advisor
{
    public class AdvisorPopup : MonoBehaviour
    {
        ...
        public void Show(Speaker speaker, string message, float delay = 1.0f)
        {
            _messageQueue.Enqueue(new QueuedMessage(speaker, message, delay));
            if (!_isProcessingQueue)
                StartCoroutine(ProcessQueue());
        }

        public IEnumerator ShowAsync(Speaker speaker, string message, float delay = 1.0f)
        {
            _messageQueue.Enqueue(new QueuedMessage(speaker, message, delay));
            if (!_isProcessingQueue)
                StartCoroutine(ProcessQueue());
        ...
        private IEnumerator ProcessQueue()
        {
            ...
            while (_messageQueue.Count > 0)
            {
                ...
                yield return ShowInternal(msg.Speaker, msg.Message, msg.Delay);
            }
        ...
        private IEnumerator ShowInternal(Speaker speaker, string message, float delay)
        {
            Init();
